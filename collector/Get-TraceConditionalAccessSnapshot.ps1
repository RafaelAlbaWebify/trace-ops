...
function New-TraceSnapshotError {
    ...
    [ordered]@{
        ...
        evidence_area = "conditional_access"
        error = [ordered]@{
            code = $Code
            message = $Message
            scenario = $Scenario
...
if ($sample.PSObject.Properties.Name -notcontains "conditional_access") {
    New-TraceSnapshotLimitation -Area "conditional_access" -Scenario $requestedScenario | ConvertTo-Json -Depth 20
    exit 0
}
...
[ordered]@{
    ...
    evidence_area = "conditional_access"
    conditional_access = $sample.conditional_access
} | ConvertTo-Json -Depth 20
