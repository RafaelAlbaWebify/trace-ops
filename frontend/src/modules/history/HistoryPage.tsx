...
function renderItem(item: unknown, index: number) {
  if (!item || typeof item !== "object") {
    return { id: index + 1, user: "unknown", service: String(item), status: "ok" };
  }
  const obj = item as Record<string, unknown>;
  return {
    id: obj.id ?? obj.run_id ?? index + 1,
    user: obj.user ?? obj.user_principal_name ?? obj.userSam ?? "unknown",
    service: obj.service ?? obj.module ?? obj.diagnostic ?? "diagnostic",
  ...
  return (
    <section className="trace-page">
      <div className="trace-page-header">
        ...
        <p>Review previous diagnostic runs and report links when available.</p>
      </div>
