...
$Files = Get-ChildItem -LiteralPath $RepoRoot -Recurse -File -Force | Where-Object { $_.FullName -notlike "*\.git\*" -and $_.FullName -notlike "*\node_modules\*" -and $_.FullName -notlike "*\.venv\*" -and $_.FullName -notlike "*\dist\*" -and $_.FullName -notlike "*\.trace-runs\*" }
...
try {
    ...
    Write-JsonFile -Object $ParseErrors -Path (Join-Path $AuditRoot "powershell-parse.json")
...
if ($null -eq $Python) {
    ...
    foreach ($File in $JsonFiles) {
        ...
        $Out = & $Python -c $Code $File.FullName 2>&1
        if ($LASTEXITCODE -ne 0) { $JsonErrors += [pscustomobject]@{ file = $File.FullName; error = ($Out | Out-String).Trim() } }
    }
    Write-JsonFile -Object $JsonErrors -Path (Join-Path $AuditRoot "json-validity.json")
...
Write-JsonFile -Object $SecretHits -Path (Join-Path $AuditRoot "secret-scan.json")
...
if (-not $SkipFrontend) {
    if (-not (Command-Exists "npm")) { Add-Check -Name "Frontend prerequisites" -Status "FAIL" -Detail "npm not found"; Add-Finding -Severity "high" -Area "Frontend" -Message "npm was not found." -Recommendation "Install Node.js or add npm to PATH." }
    else {
...
Write-JsonFile -Object $Summary -Path (Join-Path $AuditRoot "audit-summary.json")
...
New-AuditZip -FolderPath $AuditRoot -ArchivePath $ZipPath
